...
cd 'C:\Users\19195\OneDrive\Documents\University of Wisconsin\First Year\Fall Classes\Econ 712\Problem Sets\PS5'

% Pavel Brendler, Anton Babkin, 10/9/15.
% Modified by Jason Choi, 10/18/18.
% Modified by Sarah Bass, 10/4/20.
...
for tau = [0.11 0] 
    ...
    while q<nq && (abs(K1-K0)>tolk || abs(L1-L0)>tollab)
        ...
        % Prices
        r0 = alpha*(K0^(alpha-1))*(L0^(1-alpha)) - delta; % <------------------- YOUR CODE GOES HERE
        w0 = (1-alpha)*(K0^alpha)*(L0^(-1*alpha));% <------------------- YOUR CODE GOES HERE

        % Pension benefit
        b =  (tau)*L0*w0/(sum(mass(JR:J)));% <------------------- YOUR CODE GOES HERE
        ...
        for j=J-1:-1:tW+1 % age
            for ik0=1:nk        % assets today
                ...
                while ik1<nk  	% assets tomorrow
                    ...
                    cons =  b+ (1+r0)*kap0 - kap1; % <------------------- YOUR CODE GOES HERE !!

                    if cons<=0
                        util=neg;
                    else
                        util =  (cons^(1-sigma))/(1-sigma);% <------------------- YOUR CODE GOES HERE !!
                    end

                    % Right-hand side of Bellman equation
                    v0 = util + beta*v(ik1,j+1); % <------------------- YOUR CODE GOES HERE !!
        ...
        for j=tW:-1:1           % age
            for ik0=1:nk          % assets today
                ...
                while ik1 < nk  	% assets tomorrow
                    ...
                    lab = (gamma*(1-tau)*e(j)*w0-(1-gamma)*((1+r0)*kap0 - kap1))/((1-tau)*e(j)*w0); % <------------------- YOUR CODE GOES HERE !!
                    ...
                    % Instantaneous utility
                    cons =  (1-tau)*w0*e(j)*lab+(1+r0)*kap0 - kap1; % <------------------- YOUR CODE GOES HERE !!

                    if cons<=0
                        util = neg;
                    else
                        util = (((cons^gamma)*(1-lab)^(1-gamma))^(1-sigma))/(1-sigma); % <------------------- YOUR CODE GOES HERE !!
                    end
        ...
        if tau>1e-5 && q==1
            ...
            saveas(gcf,'value_func_retired.png')
    ...
    if tau ~= 0
        res_ss = [K0, L0, w0, r0, b];
        % Solve the model with social security and
        save('ss.mat');
    elseif tau ==0
        res_noss = [K0, L0, w0, r0, b];
...
set(LEG,'FontSize',14);
saveas(gcf,'social_security.png')
...
load('ss.mat','v','mass','ikgen','J','res_ss');
...
load('no_ss.mat','v','mass','ikgen','J','res_noss');
...
SS = [res_ss V1_ss W_ss]';
NOSS = [res_noss V1_noss W_noss]';
T = table(SS,NOSS);
T.Properties.RowNames = {'capital' 'labor' 'wage' 'interest' 'pension benefit' 'newborn welfare' 'aggregate welfare' };
